...
class IGSNotifyUser( Interface ):
    ...
    def send_notification(n_type, n_id='default', n_dict=None, email_only=()): #@NoSelf
        ...
        An email message is sent, from the standard support email address
        to the user's verified email addresses. If ``email_only`` is
        specified then the message is only sent to the email addresses
